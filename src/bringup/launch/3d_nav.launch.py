...
def generate_launch_description():
    ...
    scan_traversability_filter_pkg = get_package_share_directory("scan_traversability_filter")
    fast_lio_localization_pkg = get_package_share_directory("fast_lio_localization")
    grid_map_terrain_analysis_pkg = get_package_share_directory('elevation_mapping_cupy')
    local_planner_pkg = get_package_share_directory('local_planner')
    rviz_config_dir = os.path.join(get_package_share_directory('bringup'), 'rviz', '3d_nav.rviz')
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            launch_description_source=PythonLaunchDescriptionSource(
                launch_file_path=os.path.join(
                    scan_traversability_filter_pkg,
                    "launch",
                    "scan_traversability_filter.launch.py"
                )
            ),
            launch_arguments={
                'use_sim_time': 'true'
        ...
        IncludeLaunchDescription(
            launch_description_source=AnyLaunchDescriptionSource(
                launch_file_path=os.path.join(local_planner_pkg,"launch","local_planner.launch")
            ),
        ),
